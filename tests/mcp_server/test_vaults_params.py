...
def test_vault_update_params_defaults():
    ...
    params = VaultUpdateParams(vault_name="TestVault")
...
def test_vault_update_params_vault_name_required():
    """Verify vault_name is required for VaultUpdateParams."""
    try:
...
def test_vault_update_params_optional_fields():
    ...
    params1 = VaultUpdateParams(vault_name="Vault1", description="Desc")
    assert params1.vault_name == "Vault1"
    ...
    params2 = VaultUpdateParams(vault_name="Vault2", host_path="/path")
    assert params2.vault_name == "Vault2"
    assert params2.host_path == "/path"

    # Test with only container_path
    params3 = VaultUpdateParams(vault_name="Vault3", container_path="/container")
    assert params3.vault_name == "Vault3"
    assert params3.container_path == "/container"

    # Test with only force
    params4 = VaultUpdateParams(vault_name="Vault4", force=True)
    assert params4.vault_name == "Vault4"
