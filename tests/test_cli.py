...
class TestCliHelpers:
    ...
    def test_scan_vault_success(self):
        ...
        from obsidian_rag.cli_commands import _scan_vault

        with patch("obsidian_rag.cli_commands.scan_markdown_files") as mock_scan:
...
class TestFormatTaskResults:
    ...
    def test_format_task_results_empty(self):
        ...
        from obsidian_rag.cli_commands import _format_task_results
...
class TestLogLevelFlag:
    ...
    def test_log_level_debug(self):
        ...
        with patch(
            ...
            with patch("obsidian_rag.cli_commands.DatabaseManager") as mock_db_manager:
                mock_db_manager.return_value = mock_db_instance
...
class TestGetEmbeddingProvider:
    ...
    def test_get_embedding_provider_with_missing_config(self):
        ...
        from obsidian_rag.cli_commands import _get_embedding_provider

        mock_settings = MagicMock()
        mock_settings.get_endpoint_config.return_value = None

        with patch("obsidian_rag.cli_commands.ProviderFactory") as mock_factory:
...
class TestBuildTagConditionCli:
    ...
    def test_build_tag_condition_cli_returns_or_condition(self):
        ...
        assert hasattr(condition, "clauses") or self._compile_condition(condition).startswith("(")
...
def test_apply_include_tags_cli_with_tags():
    ...
    with patch("obsidian_rag.cli_commands._build_tag_condition_cli") as mock_build:
        ...
        result = _apply_include_tags_cli(query, ["work"])
    ...
    with patch("obsidian_rag.cli_commands._build_tag_condition_cli") as mock_build:
        with patch("obsidian_rag.cli_commands.or_") as mock_or:
            with patch("obsidian_rag.cli_commands.not_") as mock_not:
                ...
                result = _apply_exclude_tags_cli(query, ["blocked"])
...
def test_tasks_command_finds_by_document_tags():
    ...
    with patch("obsidian_rag.cli_commands.DatabaseManager") as mock_db_manager, \
         patch("obsidian_rag.cli_commands._build_tasks_query", return_value=mock_query) as mock_build:
        mock_settings = MagicMock()
        mock_settings.database.url = "postgresql+psycopg://localhost/test"
        mock_settings.logging.level = "INFO"
        mock_settings.logging.format = "text"
...
def test_ingest_command_accepts_force_flag():
    ...
    with patch("obsidian_rag.cli_commands._run_ingestion") as mock_run:
        with patch("obsidian_rag.cli.get_settings") as mock_get_settings:
            ...
            result = runner.invoke(cli, ["ingest", "/tmp", "--vault", "test", "--force"])
...
class TestCliForceIntegration:
    ...
    def test_ingest_with_force_flag(self, mock_resolve, mock_run):
        ...
        with patch("obsidian_rag.cli.get_settings", return_value=mock_settings):
            result = runner.invoke(cli, ["ingest", "/vault", "--vault", "test-vault", "--force"])
...
class TestQueryExactFlag:
    ...
    @patch("obsidian_rag.cli_commands.DatabaseManager")
    @patch("obsidian_rag.cli_commands._get_embedding_provider")
    def test_query_semantic_search_unchanged(
        self, mock_get_provider, mock_db_manager
    ):
