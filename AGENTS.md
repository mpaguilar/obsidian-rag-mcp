...
### 054.auth-header (Completed 2026-07-17)

**Objective:** Add a 5-layer SigV4 region resolution chain to `_write_s3()` in `obsidian_rag/mcp_server/output_file.py` so MCP `output_file={"type":"s3",...}` calls succeed against non-AWS S3-compatible servers (Garage, MinIO, Ceph) that reject the previously hardcoded `us-east-1` default on `PutObject` with `unexpected scope`.

**Changes Made:**
- **Added `region` field to `OutputFileConfig`** in `obsidian_rag/mcp_server/models.py` (per-call SigV4 signing region override, highest precedence in resolution chain).
- **Added `output_file_s3_region` field to `MCPConfig`** in `obsidian_rag/config_models.py` (deployment-wide default region; env var `OBSIDIAN_RAG_MCP_OUTPUT_FILE_S3_REGION`).
- **Added `_parse_mcp_env_vars` model_validator + `_collect_mcp_env_vars` + `_merge_mcp_env_vars` helpers to `Settings`** in `obsidian_rag/config.py` — needed because `output_file_s3_region` contains underscores in its field name, so Pydantic's `env_nested_delimiter` cannot map `OBSIDIAN_RAG_MCP_OUTPUT_FILE_S3_REGION` cleanly. The validator collects all `OBSIDIAN_RAG_MCP_*` env vars and merges them into the `mcp` dict (only filling missing/None fields, preserving kwargs precedence).
- **Implemented the 5-layer resolution chain in `output_file.py`**: `_resolve_s3_region(config, app_default_region)` returns a concrete region string by trying (1) per-call `config.region`, (2) `app_default_region`, (3) URL-derived region via `_derive_region_from_endpoint`, (4) `GetBucketLocation` probe via `_probe_bucket_region` (non-AWS endpoints only, signed with `us-east-1`, best-effort silent fall-through), (5) `us-east-1` fallback + WARNING via `_warn_fallback_region`. URL-derivation is attempted before the probe for efficiency (local string parsing before a network round-trip); the probe only fires when URL-derivation returns None.
- **`_derive_region_from_endpoint`**: regex table (`_ENDPOINT_REGION_PATTERNS`) recognizing AWS hostnames (`s3.<region>.amazonaws.com`, `s3-<region>.`, `s3-website.<region>.`), DigitalOcean Spaces, Wasabi, Backblaze B2; Cloudflare R2 → `"auto"`; bare `s3.amazonaws.com` → `"us-east-1"` (special-case so AWS us-east-1 callers get no WARNING). Returns None for unrecognized/custom endpoints (Garage, MinIO, IP addresses).
- **`_probe_bucket_region`**: signs `GetBucketLocation` with `us-east-1`, returns the `LocationConstraint` if non-empty string, else None. Catches `(ClientError, OSError, ConnectionError)` and returns None (best-effort, never propagates). Uses normal project TLS (NO `verify=False`) — if the target uses an internal CA, the probe fails at handshake and falls through to fallback + WARNING; the operator then sets `region` explicitly. Added `bucket: str` parameter (required by `get_bucket_location(Bucket=...)`).
- **`_warn_fallback_region`**: emits a WARNING log naming the endpoint and suggesting `OutputFileConfig.region` or `OBSIDIAN_RAG_MCP_OUTPUT_FILE_S3_REGION` env var. Pre-formatted `_msg` variable per CONVENTIONS.
- **Threaded `region_name` into `_write_s3()`** and `boto3.client("s3", ..., region_name=resolved)` call. Added `app_default_region: str | None = None` parameter to `write_output_file()` dispatcher (avoids import cycles).
- **Threaded `app_default_region` into all 7 tool wrappers**: server.py (query_documents, get_documents_by_tag, get_documents_by_property, get_all_tags, get_tasks) and document_tools.py (get_document, list_documents). Extracted `_dispatch_output_file` helper in server.py to keep server.py under 1000 lines (each callsite reduced from 8 lines to 1). All 7 tool wrapper docstrings updated with a one-line note about the `region` field.
- **Updated `ARCHITECTURE.md`**: Added "S3 region resolution" paragraph documenting the 5-layer chain; added `region` field to OutputFileConfig field list; updated `_write_s3()` bullet to document `region_name` threading and the helper functions.
- **Tests**: New files `tests/mcp_server/test_output_file_region_probe.py` (13 tests covering probe success/failure/empty/None constraint/network error/URL-derivation-skip/us-east-1-signing/no-verify-false/`_write_s3` region threading/resolution chain branches/endpoint derivation edge cases), `tests/mcp_server/test_output_file_region_integration.py` (5 backward-compat and end-to-end dispatcher tests), `tests/test_config.py` (7 tests for `MCPConfig.output_file_s3_region` default/env-override/config-file-override + the `_parse_mcp_env_vars` validator edge cases). Extended `tests/mcp_server/test_output_file.py` (~30 region-related tests covering `_resolve_s3_region` all branches, `_derive_region_from_endpoint` all patterns, `_warn_fallback_region` presence/absence, `_probe_bucket_region` success/failure), `test_output_file_models.py` (region field default/set/serialization/None), `test_output_file_s3_integration.py` (updated for new region threading), `test_server_output_file_tools.py` and `test_document_tools_output_file.py` (assert `app_default_region` threaded).

**Key Design Decisions:**
- **URL-derivation before probe (efficiency)**: The plan suggested `probe → URL-derived`, but the implementation swaps these for efficiency — local hostname parsing is O(1) string ops, while the probe is a network round-trip. The probe only fires when URL-derivation returns None (i.e. non-AWS endpoints). Behaviorally equivalent for all practical cases (AWS endpoints resolve via URL-derived; non-AWS endpoints fall to probe then fallback). ARCHITECTURE.md documents the actual implemented order.
- **`_dispatch_output_file` helper extraction**: server.py was at 988 lines before this checkpoint. Adding 5 docstring blocks + 5 callsite expansions (8 lines each) pushed it to 1043 lines, exceeding the 1000-line limit. Extracted the `parse_output_file + write_output_file + app_default_region` pattern into a single `_dispatch_output_file(result, output_file, registry)` helper, reducing each callsite from 8 lines to 1. Compressed the 5 region docstring blocks to single-line notes merged into the preceding `output_file` param description. Final server.py: 994 lines.
- **Custom env var validator for MCP**: `output_file_s3_region` contains underscores, so Pydantic's `env_nested_delimiter` (single underscore) cannot map `OBSIDIAN_RAG_MCP_OUTPUT_FILE_S3_REGION` cleanly. Added a `model_validator(mode="before")` that collects all `OBSIDIAN_RAG_MCP_*` env vars and merges them into the `mcp` dict, filling only missing/None fields (preserves kwargs/config-file precedence). This validator is general — it handles any future `OBSIDIAN_RAG_MCP_*` env var with underscores in the field name.
- **Broad `except (ClientError, OSError, ConnectionError)` in `_probe_bucket_region`**: Intentional — the probe is best-effort and must never propagate. Catches the specific failure modes (AWS API error, network/OS error, connection error) rather than a bare `Exception` to satisfy BLE001. The probe failing on any error and returning None is the documented contract.
- **Bare `s3.amazonaws.com` special-case**: Returns `"us-east-1"` (AWS's canonical region for bare hostnames) so legitimate AWS us-east-1 callers get no WARNING. Documented in the `_ENDPOINT_REGION_PATTERNS` table and ARCHITECTURE.md.
- **No `verify=False`**: The probe uses boto3's default TLS verification. If a target has an internal CA (like the Garage diagnostic endpoint), the probe fails at TLS handshake and falls through — the operator then sets `region` explicitly. The probe is a convenience, not a security boundary.

**No breaking changes:** Default `region=None` preserves all existing behavior for AWS callers (URL-derived region or `us-east-1` fallback, identical to before). No schema changes, no Alembic migration, no config file changes. Only NEW behavior: non-AWS S3-compatible servers (Garage/MinIO/Ceph) now get a working region resolution chain instead of a hardcoded `us-east-1` that was rejected on `PutObject`.

**Verification:**
- All 2502 tests pass (1 skipped pre-existing)
- 100% code coverage (5847 statements, 1192 branches)
- All ruff checks pass; ruff format clean (234 files)
- mypy clean on source code (59 source files, no issues)
- bandit clean (0 violations)
- All source files under 1000 lines (max: ingestion.py at 997, server.py at 994, output_file.py at 390)
- No `# noqa` in source code
- Code review: APPROVED (1 easy CHANGES REQUESTED — ARCHITECTURE.md chain-order documentation mismatched the implemented code order; fixed by reordering the docs to match `URL-derived → probe` implementation, and clarifying bare `s3.amazonaws.com` → `us-east-1` special-case)

### 050.parser-replacement (Completed 2026-07-08)

**Objective:** Replace the regex-based `_strip_code_blocks()` in `obsidian_rag/parsing/body_tags.py` with a mistune-based pure parser-children AST walk so inline `#tag` patterns are extracted only from prose (paragraphs, blockquotes, list items — NOT headings) and reliably excluded from ALL code constructs (fenced blocks of any backtick length, indented 4-space code blocks, inline code spans), with escaped `\#tag` and decimal `&#35;tag` cleanly excluded via mistune's token-splitting behavior.

**Changes Made:**
- **Added `mistune>=3.0` to `pyproject.toml`** as a DIRECT dependency (REQ-007) and ran `uv sync`. mistune 3.3.2 is now recorded as a direct (not transitive) dependency in `uv.lock`. mistune has zero transitive dependencies (no new packages added to the lockfile beyond mistune itself).
- **Rewrote `obsidian_rag/parsing/body_tags.py`** (128 → 169 lines): Removed `FENCED_CODE_PATTERN`, `INLINE_CODE_PATTERN`, `PROSE_MENTION_PATTERN` module-level regexes and the regex-based `_strip_code_blocks()` function. Retained `INLINE_TAG_PATTERN` (the Obsidian tag regex) and `_collect_unique_tags()` (now scans parser-filtered prose, parameter renamed `stripped` → `prose_text` with updated docstring). Added module-level singleton `_MD_AST = mistune.create_markdown(renderer="ast")` (Detail Q5 — amortizes construction cost during bulk ingestion). Added `_PROSE_BLOCK_TYPES` frozenset (`paragraph`, `block_quote`, `list`, `list_item`, `block_text`) and `_SKIP_BLOCK_TYPES` frozenset (`block_code`, `heading`, `blank_line`, `thematic_break`, `inline_html`). Implemented recursive AST walk split into two helpers to keep McCabe complexity ≤ 5: `_walk_block()` (recurses prose containers via `_walk_inline`, skips code/heading/blank/break/html tokens, defensively recurses unknown block types with children) and `_walk_inline()` (collects `text` token `raw` strings, skips `codespan`, recurses `emphasis`/`strong`/`link`/`image`/etc children). Added `_extract_prose_text()` entry point that walks the top-level block list and joins collected `text` raws. Rewrote `extract_body_tags()` to: None/empty/whitespace → None; parse body via `_MD_AST`; defensively guard against unexpected `str` output from parser; collect prose texts; join with `\n`; scan with `_collect_unique_tags`; return `None` if no tags. Public signature `(content: str | None) -> list[str] | None` preserved (REQ-003); `ingestion.py:586` call site unchanged. Added docstrings with Args/Returns to `_walk_block`, `_walk_inline`, `_extract_prose_text`, `_collect_unique_tags`, `extract_body_tags`. Logging via pre-formatted `_msg` variables per CONVENTIONS.
- **Rewrote `tests/test_parsing_body_tags.py`** (171 → ~190 lines, function-based per CONVENTIONS): Removed `TestStripCodeBlocks` class and `TestExtractBodyTags` class. Removed all 10 `_strip_code_blocks` tests (function removed). Converted `TestExtractBodyTags` methods to module-level `def test_*` functions. Updated `test_extract_body_tags_heading_excludes_tag_same_line` for Q1 (`"# Heading with #tag"` → `None`, was `["tag"]`). Dropped `_strip_code_blocks` from imports.
- **Updated `tests/test_parsing_body_tags_prose_mentions.py`**: Removed `_strip_code_blocks` import. Rewrote `test_inline_code_does_not_cross_newlines` to assert on `extract_body_tags` only (no `_strip_code_blocks` call). Rewrote `test_prose_mention_with_language_id_inline` to assert `extract_body_tags` only. Rewrote `test_double_quoted_hash_prose_is_valid_tag` to drop the `_strip_code_blocks` assertion. Updated `test_prose_mention_empty_triple_backticks` for Q3 (`"``` ``` ```\n#tag\n"` → `["tag"]`, was `None`). The remaining 049 regression tests (no garbage, prose mention clean) pass unchanged.
- **Updated + extended `tests/test_parsing_body_tags_edge_cases.py`**: Updated `test_heading_with_tag_same_line` for Q1 (`"# Heading text #tag"` → `None`, was `["tag"]`). Added 6 new function-based tests: `test_four_backtick_fence_tags_excluded` (4-backtick fence length-matching — NEW capability the old regex could not express), `test_indented_code_block_tags_excluded` (4-space indented code — NEW capability the old regex did NOT handle), `test_setext_heading_excludes_tag` (setext `Heading #tag\n===` → `None`), `test_escaped_tag_excluded` (`\#tag and #real` → `["real"]`), `test_decimal_entity_tag_excluded` (`&#35;tag` → `None`), `test_hex_entity_tag_limitation` (`&#x23;tag` → `["x23"]` — documents the accepted limitation).
- **Updated `ARCHITECTURE.md`** "Body Tag Extraction" section: Replaced the 4-layer regex `_strip_code_blocks()` description with the new mistune-based AST-walk design. Documented: (a) `mistune.create_markdown(renderer='ast')` parses body into block-token dicts; recursive walk extracts tags ONLY from `text` inline children inside `paragraph`/`block_quote`/`list`/`list_item`/`block_text`; (b) `block_code` (fenced any backtick length + indented 4-space) and `codespan` skipped entirely; structurally eliminates 049 garbage-tags bug; (c) headings excluded entirely (ATX + setext); (d) escaped `\#tag` cleanly excluded via token split; (e) decimal `&#35;tag` excluded by `;` boundary + `isdigit()`; (f) hex `&#x23;tag` leaks `x23` (documented limitation); (g) degenerate `` ``` ``` ``` `` returns `["tag"]` (more correct than old `None`).

**Key Design Decisions:**
- **mistune over markdown-it-py (REQ-007/Q4)**: markdown-it-py NORMALIZES escapes and entities into `#` inside its text tokens (leaks `\#tag`/`&#35;tag` as tags — would have forced a hybrid regex approach). mistune SPLITS escaped `\#` into a separate `text` token `{"raw": "#"}` (no tag chars follow → no match) and PRESERVES decimal `&#35;tag` literally (regex matches `#35`, all-numeric → excluded by `isdigit()`). mistune returns plain dicts (`renderer='ast'`) — trivially walkable with simple key checks; marko returns a class hierarchy requiring `isinstance` checks (more verbose). mistune is pure Python with zero transitive dependencies.
- **Module-level singleton `_MD_AST` (Detail Q5)**: `mistune.create_markdown(renderer="ast")` is constructed once at module import and reused across all `extract_body_tags` calls. Amortizes parser construction cost during bulk ingestion. A pure module-level callable; no class state.
- **Two-function walk split (`_walk_block` + `_walk_inline`)**: mistune's AST has block tokens whose children are a MIX of block tokens (e.g. `block_quote` → `paragraph`) and inline tokens (e.g. `paragraph` → `text`/`codespan`/`emphasis`). Splitting into `_walk_block` (delegates prose containers to `_walk_inline`, recurses unknown block types) and `_walk_inline` (collects `text.raw`, skips `codespan`, recurses emphasis/strong/link children) keeps each function's McCabe complexity ≤ 5 (CONVENTIONS.md max). A single-function walk would exceed complexity 5.
- **Per-`text`-token scan via concatenation**: `_extract_prose_text` collects all eligible `text` token `raw` strings into a list; `extract_body_tags` joins them with `\n` and calls `_collect_unique_tags` once. This preserves dedup ordering across the whole document and keeps `_collect_unique_tags` unchanged (single responsibility). Scanning per-token would also work but would require threading dedup state through the walker.
- **Defensive `isinstance(ast, str)` guard**: mistune's AST renderer returns `list[dict]` for valid markdown, but the type annotation allows `str | list[dict]`. The guard returns `None` for unexpected string output (unreachable in practice with `renderer='ast'` but covers the typed return contract). Covered by test mocking `_MD_AST` to return a string.
- **`block_text` in `_PROSE_BLOCK_TYPES` is REQUIRED**: tight lists emit `list_item` → `block_text` → `text` children (not `paragraph`). Without `block_text` in the prose set, task tags in tight lists (`- [ ] do something #important`) would be lost. Verified against mistune 3.3.2.
- **Heading exclusion (Q1/REQ-005)**: The problem statement lists prose = paragraphs/blockquotes/list-items; headings are NOT listed. `# Heading text #tag` → `None` (was `["tag"]`). This is a documented behavioral change — re-ingestion with `force=True` rewrites `Document.tags` for affected documents.
- **Hex entity `&#x23;tag` leak accepted (Q2/REQ-002)**: mistune preserves `&#x23;tag` literally; regex matches `#x23` (NOT all-numeric) → `x23` extracted as a false-positive tag. Extremely rare in real Obsidian notes; accepted as documented limitation per Q2 Option A. The clean handling of `\#tag` and `&#35;tag` (far more common) is the win.
- **Degenerate triple-backtick returns `["tag"]` (Q3/REQ-006)**: `extract_body_tags("``` ``` ```\n#tag\n")` → `["tag"]`. mistune parses the backticks as `codespan`/`text`/`softbreak`/`text` children of a `paragraph`; `#tag` is in a `text` child → genuinely in prose. Arguably more correct than the old `None`.

**No breaking changes for API:** `extract_body_tags` signature and `None`/empty/whitespace contract preserved (REQ-003). No schema changes, no Alembic migration, no config changes, no API changes. **Behavioral changes (correctness improvements)** for documents containing: (a) tags inside indented code blocks (previously WRONGLY extracted — now correctly excluded), (b) tags inside 4+ backtick fences (previously mis-stripped — now correctly excluded), (c) `#tag` inside headings (previously extracted — now excluded per Q1), (d) escaped `\#tag` (previously extracted as `tag` — now excluded cleanly), (e) decimal `&#35;tag` entities (previously extracted — now excluded). These change stored `Document.tags` on re-ingest. Re-ingestion with `force=True` is the migration path for already-stored data (unchanged from checkpoint 049's cleanup path). No Alembic migration required — `Document.tags` is `TEXT[]` fully rewritten on each ingest.

**Verification:**
- All 2332 tests pass (1 skipped pre-existing)
- 100% code coverage (5549 statements, 1112 branches)
- All ruff checks pass; ruff format clean (217 files)
- mypy clean on source code (57 source files, no issues)
- bandit clean (0 violations)
- All source files under 1000 lines (max: ingestion.py at 999; body_tags.py at 169)
- No `# noqa` in source code
- Plan smoke test (16 assertions covering all REQs) passes
- Code review: APPROVED (3 easy CHANGES REQUESTED — missing docstrings on `_walk_block`/`_walk_inline` and stale docstring/param name on `_collect_unique_tags`; all fixed and re-verified)
